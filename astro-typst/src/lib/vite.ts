...
export default function (config: AstroTypstConfig): Plugin {
    ...
    const plugin: Plugin = {
        ...
        async transform(_: string, id: string) {
            ...
            const isBody = opts.includes("body") || config.body || "hast";
            let { html, getFrontmatter } = await renderToHTMLish(
                {
                    mainFilePath,
                    body: emitSvg ? "hast" : isBody,
                },
                config.options,
                isHtml
            );
            ...
            if (emitSvg && !isHtml) {
                ...
                html = imgSvg;
            } else if (isHtml) { // output is hast by default
                if (typeof html !== 'string') {
                    html = await rehypeTypstx(html, astroConfig.markdown?.rehypePlugins ?? []);
                    console.log("Transformed HTML:", html.slice(0, 2000) + "...");
                }
            }

            let code: string;
            if (isHtml && isBody === "hast") {
