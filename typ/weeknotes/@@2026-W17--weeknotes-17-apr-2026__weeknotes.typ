...
```
  ...
  Validness should at least mean not raising a type error when filled in, so let's take that as a start. What about being principal? Intuitively, just like in systems with subtyping we pick the type satisfying certain properties with regard to the subtyping relation, in a gradual type system we should pick the type with certain properties with regard to the _precision_ or _materialization_ relation. Also like subtyping, the precision or materialization relation induces a #link("https://en.wikipedia.org/wiki/Semilattice")[upper semilattice] on the gradual types. So, the problem becomes: given a set $T$ of members of the semilattice $chevron.l S, <= chevron.r$, pick a $t in T$ satisfying certain properties. Here the relation $a <= b$ means $a$ is more precise than $b$, or $a$ materializes $b$.
